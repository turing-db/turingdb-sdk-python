#!/usr/bin/env python3
"""
TuringDB Shell Client - A minimal database shell example
"""
...
def start_shell(client: TuringDB):
    ...
    shell_commands = {
        "cd": change_graph,
        "list-avail-graphs": list_available_graphs,
        "list-loaded-graphs": list_loaded_graphs,
    }
...
def show_help():
    ...
    <cyan>list-avail-graphs</cyan>         - List available graphs
    <cyan>list-loaded-graphs</cyan>        - List loaded graphs
...
def change_graph(client: TuringDB, args: list[str]):
    ...
    try:
        ...
        print_formatted_text(HTML(f"<green>✓ Changed graph to {graph_name}</green>"))
    except TuringDBException as e:
        print_formatted_text(HTML(f"<red>{e}</red>"))
